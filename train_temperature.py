...
def train_temperature():
    ...
    df = df.dropna()
    X = df[["humidity", "lux", "output_current", "charging_current"]]  # bez 'temperature'
    y = df["temperature"]
    ...
    try:
        ...
        (psycopg2.Binary(model_bytes), "model_temperature"))
        cursor.execute(
            "UPDATE models SET model_data = %s WHERE name = %s",
            (psycopg2.Binary(scaler_bytes), "model_temperature_scaler"))
        conn.commit()
        cursor.close()
        conn.close()
        st.success("✅ Zapisano model_temperature i scaler.")
    except Exception as e:
