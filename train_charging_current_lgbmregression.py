...
print("✅ Model gotowy.")
print(f" Czas trenowania: {train_duration:.2f} sekundy")
...
try:
    ...
    cursor = conn.cursor()
    cursor.execute("INSERT INTO models (name, model_data) VALUES (%s, %s)", ("model_charging_lgbmregres", psycopg2.Binary(model_bytes)))
    cursor.execute("INSERT INTO models (name, model_data) VALUES (%s, %s)", ("model_charging_lgbmregres_scaler", psycopg2.Binary(scaler_bytes)))
    conn.commit()
    cursor.close()
    conn.close()
